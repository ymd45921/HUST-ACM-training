...
signed main()
{
    ...
    const auto fixSeg = [&](pair<longd, longd> &seg) -> void
    {
        if (seg.first <= 0) maximize(seg.second, min(n, -seg.first));
        if (seg.second >= n) minimize(seg.first, max(0.l, 2 * n - seg.second));
    };
    const auto check = [&](longd time) -> bool
    {
        auto [p1, v1] = a1;auto [p2, v2] = a2;
        auto s1 = make_pair(p1 - v1 * time, p1);
        auto s2 = make_pair(p2, p2 + v2 * time);
        fixSeg(s1), fixSeg(s2);
    ...
    const auto metInCenter = [&]() -> longd
    {
        ...
        auto pos = p1 + time * v1;
        return time += max(pos / v1, (n - pos) / v2);
    };
    for (cin >> t; t --;)
    {
