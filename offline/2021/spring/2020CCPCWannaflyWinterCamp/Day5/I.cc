...
namespace ST {
    ...
    struct ST {
        ...
        longs ask(int l, int r) {
            uint k = lg[r - l + 1];
            return max(f[k][l], f[k][r - (1u << k) + 1]);
        }

        void build(int n) {
            for (int i = 1; i < LG; ++ i)
                for (int j = 1; j + (1u << i) - 1 <= n; ++ i)
                    f[i][j] = max(f[i - 1][j], f[i - 1][j + (1u << i) - 1]);
        }
    };
}
...
signed main() {
    ...
    int n, m1, m2, x1, x2, y1, y2, w;
    input, n, m1, m2;
    while (m1 --) {
        input, x1, y1, x2, y2, w;
        pre[x2][y2] += w, pre[x1][y1] += w;
        pre[x2][y1] -= w, pre[x1][y2] -= w;
    }
    ST::ST st[N]; ST::init();
    for (int i = 1; i <= n; ++ i)
        for (int j = 1; j <= n; ++ j)
            st[i].f[0][j] = pre[i][j] + pre[i - 1][j - 1] - pre[i][j - 1] - pre[i - 1][j];
    for (int i = 1; i <= n; ++ i)
        st[i].build(n);
    while (m2 --) {
        input, x1, y1, x2, y2;
