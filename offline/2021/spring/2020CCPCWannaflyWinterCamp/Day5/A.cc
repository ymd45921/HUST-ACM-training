...
signed main() {
    ...
    input, n, k;
    for (int i = 0; i < k; ++ i) {
        $(m).nextArray(a[i], a[i] + m);
        for (int j = 0; j < m; ++ j)
            part[a[i][j]].set(i);
    }
    for (int i = 1; i <= n; ++ i) {
        p[part[i].to_ulong()].set(i);
    }
    for (int i = 0; i < 8; ++ i)
        cnt[i] = (int)p[i].count();
    maximize(ans, cnt[7]);
    const auto match = [&](int i, int j) {
        int matched = min(cnt[i], cnt[j]);
        ans += matched, cnt[i] -= matched, cnt[j] -= matched;
    };
    match(1, 6), match(2, 5), match(4, 3);
    ans += max({cnt[1], cnt[2], cnt[4]});
