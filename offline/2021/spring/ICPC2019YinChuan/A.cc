...
using member = tuple<string, string, int>;
...
signed main() {
    ...
    for (cin >> T; T --;) {
        ...
        while (!value.empty())
            value.pop();
        for (cin >> n; n --;) {
            cin >> name >> color >> appeal;
            a.emplace_back(name, color, appeal);
        }
        for (int i = 5; -- i;) {
            cin >> burst;
            bonus.insert(burst);
        ...
        for (auto [nm, cl, ap] : a) {
            double multi = 1.0;
            if (bonus.count(nm)) multi += 0.1;
            if (cl == burst) multi += 0.2;
            value.push(int(multi * ap));
        }
        longs ans = 0;
        for (int i = 5; i --; value.pop()) {
            ans += value.top();
        }
        cout << ans << endl;
    }
