...
//        visit[nowpos] = true;
        if(dist[nowpos]<nowdis)continue;
        longs costs;
        ...
        for(auto i : edgelist[nowpos])
        {
            ...
            if(dist[i.togo]>dist[nowpos]*costs)
            {
                dist[i.togo] = dist[nowpos]*costs;
                table.push({i.togo,nowlvl+i.lvl,dist[i.togo]});
            }
        }
    }
