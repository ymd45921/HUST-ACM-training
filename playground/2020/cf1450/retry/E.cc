 ...
 * 1. 什么是差分约束系统？
 * 
 * - 如果一组不等式构成的系统由 n 个变量和 m 组关系组成
 * - 每组关系式是形如 a[i] - a[j] <= x 的不等式
 * - 求解的是某个式子的最大值、最小值或者是该系统是否有解
 * 
 * 2. 如何解决这种问题？
 * 
 * - 建图，变量对应节点，关系对应图中的连边（推广三角不等式
 * - 求式子 a[i] - a[j] 的最大值：求 i - j 的最短路
 * - 求式子 a[i] - a[j] 的最小值：求 i - j 的最长路（SPFA 反向松弛就行，对应另一种差分约束系统
 * - 建图后未必存在最短/最长路；因为可能存在无限的负环/正环
 * 
 * 3. 做这种题目一般需要注意什么？
 * 
 * - 
