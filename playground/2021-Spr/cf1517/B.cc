...
signed main() {
    ...
    while (T --) {
        ...
        for (int i = 1; i <= n; ++ i)
            $.nextArray(b[i] + 1, b[i] + 1 + m);
        for (int i = 1; i <= n; ++ i)
            for (int j = 1; j <= m; ++ j)
                p.push({-b[i][j], i});
        for (int i = 1; i <= n; ++ i)
            memset(ans[i], 0, sizeof(int) * (m + 1));
        fill(cur + 1, cur + 1 + n, 1);
        for (int i = 1; i <= m; ++ i) {
            auto [cost, sta] = p.top();
            p.pop(), cost = -cost;
            ans[sta][i] = cost;
        ...
        while (!p.empty()) {
            auto [cost, sta] = p.top();
            p.pop(), cost = -cost;
            while (ans[sta][cur[sta]]) ++ cur[sta];
            ans[sta][cur[sta]] = cost, ++ cur[sta];
        }
        for (int i = 1; i <= n; ++ i)
            $.putArray(ans[i] + 1, ans[i] + 1 + m);
    }
