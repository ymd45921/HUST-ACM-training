...
{
    int ll = getLayer(lhs), rl = getLayer(rhs);
    if (ll != rl) return ll < rl;
    else return getId(lhs) < getId(rhs);
}
...
signed main()
{
    ...
    while (T --)
    {
        ...
        for (int i = 1; i <= n; ++ i)
        {
            const auto &last = a[i - 1], &now = a[i];
            int ll = getLayer(last), nl = getLayer(now);
            if (ll == nl)
                ans += !(ll % 2) * (getId(now) - getId(last)) / 2;
            else
            {
                int d = nl - ll;
                ans += d / 2 + (d % 2) * (ll % 2);
