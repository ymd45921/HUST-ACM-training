...
signed main() {
    ...
    m.reserve(p.size() * 2);
    int tag = 0;
    for (auto pp : p) m[pp] = tag ++;

    $(n, q).nextArray(a + 1, a + 1 + n);
    for (int i = 1; i <= n; ++ i) {
        for (auto j = a[i]; j > 1; j /= v[j])
            aa[i].set(m[v[j]]);
    }
    vector<int> split;
    for (int i = 1; i <= n; ++ i) {
        if (!(tmp & aa[i]).count()) tmp |= aa[i];
        else {
            split.push_back(i);
            tmp = aa[i];
        }
    ...
    while (q--) {
        $(l, r);
        auto rr = upper_bound(split.begin(), split.end(), r);
        auto ll = lower_bound(split.begin(), split.end(), l);
        $.put(rr - ll + 1);
    }
