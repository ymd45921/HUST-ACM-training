...
signed main() {
    ...
    pair<int, int> toDelete(2060, 0);
    for (int i = 1; i <= n; ++ i)
        minimize(toDelete, make_pair(a[i], i));
    auto sum = accumulate(a + 1, a + 1 + n, 0);
    if (sum % 2) return $.put(0), 0;
    auto half = sum / 2;
    f[0] = true;
    for (int i = 1; i <= n; ++ i)
        for (int j = sum; j >= 0; -- j)
    ...
    if (f[half]) {
        ...
        while (base < N) {
            for (int i = 1; i <= n; ++ i)
                if ((sum - a[i]) / base % 2)
                    return $.put(1).put(i), 0;
            base <<= 1u;
        }
        $.put(1).put(toDelete.second);
    } else $.put(0);
