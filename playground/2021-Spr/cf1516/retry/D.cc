/**
 *
 * 虽然 DP 是应该考虑到的，但是还要考虑使用倍增的技巧；
 * 
 * 首先不难想到将 N 内所有的数字分解质因数；
 * 然后每个位置 i 开始可以到达最远的互质就是所有出现的这些因子的下一个位置的最小值
 * 那么，接下来就是确定一些和实现方面相关的问题：
 *   怎么维护这个下一个位置？开 M 个数组然后在数组上二分？ 
 *   - 大可不必，从后向前对于每个质数维护一个位置即可（
 *   就算这样维护了区间，区间内部的元素还是可能产生冲突啊？
 *   - 问题不大，同样是倒着更新，再考虑从下一个位置转移就可以保证了？
 *   那么维护完了，我要怎么在这个数组上快速的转移？毕竟可以被卡成 O(r-l)
 *   - 比较有意思的技巧：使用倍增，或者事二进制拆位的思想
 *   - 首先，我们已经维护了下一个可以容忍的位置的跳表
 *   - 我们可以预先处理 log n 个，容忍下 2 ** x 个位置的跳表
 *   - 查询的时候，在每一个位置上使用跳表查询，然后跳转即可
 *   综上所述，我们就可以得到这个题目的实现方法
 * 说是这么说，但是如果连倒着 DP 都没有考虑到，那就是在是没有什么办法了（
 */
#include <bits/stdc++.h>
