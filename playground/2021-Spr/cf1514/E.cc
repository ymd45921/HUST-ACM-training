 ...
 * 
 * 哈密顿路：通过图的每个节点一次且仅一次
 * 寻找一张图中的哈密顿路是 NP-complete 的
 * 哈密顿环：首尾相连，哈密顿图的标志
 * 
 * 判定：
 * - 哈密顿回路的存在：Dirac 定理（充要条件）
 *   对于 N 个节点的图，所有点的度数大于 ceil(N/2)
 * - 哈密顿路的存在：竞赛图
 *   N >= 2 的竞赛图一定存在哈密顿通路
 * 对于竞赛图，可以使用归并排序的方式确定其中的哈密顿路
 * 将每一条有线边看作偏序关系，使用归并排序排序即可
 * 
 * 基本思路就是找到最少的一些边，只是用这些边完成转移
 * 因为图是竞赛图，所以一定存在哈密顿通路，可以偏序转移
 * 我们假设我们得到的偏序关系可以从低位置到达高位置
 * 所以接下来需要考虑反向边：
 * - 显然，一个点偏序前面的所有点是 [不可达 | 可达] 的
 * - 我们只需要维护一个前驱指针，指向目前可以到达的最低节点
 * 显然，个人认为标程的实现还是没有那么美好的，但是确实是 2n 的
 *   每个位置：都有一个最大前驱，询问返回的是 0
 *   对于每个前驱位置，都会向左移动一次，询问返回的是 1
 * 大概可以缓存前缀位置，减少一定的询问次数吧（）虽然不明显就是了
 * 更新邻接矩阵的时机是前驱和当前位置重合的情况，将左下角矩阵清除
 * 
 * C++ 学习笔记：
 * - stable_sort 内部实现是 merge_sort_with_buffer
 * - vector::assign 使用前不需要清空 vector
 */
#include <bits/stdc++.h>
...
bool askTheEdge(int a, int b) {
    int res = $.put(1, a, b).flush().nextInt();
    if (res == -1) exit(EXIT_SUCCESS);
    else return (bool) res;
}

bool askEdges(int a, const vector<int> &b) {
    int res = $.print(2, ' ', a, ' ', b.size(), ' ').
                putArray(b.begin(), b.end()).flush().
                nextInt();
    if (res == -1) exit(EXIT_SUCCESS);
    else return (bool) res;
}
...
signed main() {
    ...
    while (T --) {
        ...
        for (int i = 0; i < n; ++ i)
            g[i].reset(), g[i] = ~g[i];
        vector<int> path(n), tmp;
        iota(path.begin(), path.end(), 0);
        stable_sort(path.begin(), path.end(), askTheEdge);
        int p = n - 2;
        const auto makeTmp = [&](int p) -> vector<int>& {
            tmp.assign(path.begin(), path.begin() + p + 1);
            return tmp;
        };
        for (int i = n - 1; i >= 0; -- i) {
            if (p == i) {
                for (int j = 0; j <= i; ++ j)
                    for (int k = i + 1; k < n; ++ k)
                        g[path[k]][path[j]] = false;
                -- p;
            }
            while (askEdges(path[i], makeTmp(p))) -- p;
        }
        $.put(3).putArray(0, n, [&](int i, cquery $) {
            $.putArray(0, n, [&i](int j, cquery $) {
                $.print(g[i].test(j) ? 1 : 0);
            }, "");
        }, "").flush();
        int res = $.nextInt();
        if (-1 == res) exit(EXIT_SUCCESS);
    }
