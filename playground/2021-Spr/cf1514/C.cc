/**
 *
 * bonus solution: https://codeforces.com/blog/entry/89810?#comment-782069
 *
 * 首先，因为 gcd(a % b, b) = gcd(a, b)，所以不能包含任何的非互质数
 * 然后，再过滤掉唯一的乘积即可；因为此时相当于费马小定理，一定可以倒回去
 * 具体定理名字我忘记叫啥了，，数论学的还是太不扎实了，，但是可以反证法（
 */
#include <bits/stdc++.h>
...
signed main() {
    ...
    if (tmp != 1) ans.erase((int)tmp);
    $.put(ans.size()).putArray(ans.begin(), ans.end());
