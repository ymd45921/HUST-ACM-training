...
int ls[110], rs[110], d[110], refer[110];
...
signed main()
{
    ...
    seg_tree t(110, 0);
    t.build();
    const function<int(int, int, int)> build =
    [&](int l, int r, int depth) -> int
    {
        if (l >= r) return 0;
        auto m = refer[t.query(l, r)];
        d[m] = depth;
        ls[m] = build(l, m, depth + 1);
        rs[m] = build(m + 1, r, depth + 1);
        return m;
    };
    while (T --)
    {
