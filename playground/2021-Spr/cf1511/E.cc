...
signed main() {
    ...
    for (int i = 1; i <= n; ++ i) {
        s[i].resize(m + 1);
        $.nextArray(++ s[i].begin(), s[i].end());
    }
    const auto validator = [&](int i, int j) {
        if (i < 1 || i > n || j < 1 || j > m) return false;
        return s[i][j] == 'o';
    };
    longs ans = 0;
    const auto add = [&](int cnt) {
        if (cnt < 0) return;
    ...
    for (int i = 1; i <= n; ++ i)
        for (int j = 1; j <= m; ++ j)
            if (s[i][j] == 'o') {
                if (validator(i + 1, j)) {
                    int deduct = 2;
                    deduct += validator(i + 2, j);
                    deduct += validator(i - 1, j);
                    add(oCnt - deduct);
                }
                if (validator(i, j + 1)) {
                    int deduct = 2;
                    deduct += validator(i, j + 2);
                    deduct += validator(i, j - 1);
                    add(oCnt - deduct);
                }
            }
    $.put(ans);
