...
signed main() {
    ...
    while (q --) {
        ...
        auto ll = lower_bound(c, c + n, l + 1);
        auto eq = upper_bound(c, c + n, l + 1);
        auto rr = upper_bound(c, c + n, r);
        auto same = eq - ll, more = rr - eq;
        if (!more) $.print(same % 2 ? 'A' : 'B');
        else {
