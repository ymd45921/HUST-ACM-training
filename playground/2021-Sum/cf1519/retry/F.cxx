 ...
 * 
 * 每次不能转移最大流量：
 * - 假设，有 i1 -> j 需要借助所有的钥匙才能全部流空
 * - 那么如果前面都是能转移则全部转移，还有余量的钥匙一定是最后一把
 * - 这样，对于其他的箱子而言可能并不优
 * 因此，这是错误的（
 */
#include <bits/stdc++.h>
...
signed main() {
    ...
    int ans = inf; dp[0] = 0;
    const auto update = [&](const state &now, int pay) {
        if (now.flow == va) minimize(ans, pay);
        if (now.rt < m) {
            auto id = hash<state>{}(now);
            minimize(dp[id], pay);
        }
    };
    const auto moveto = [&](const state &s, state &nxt) {
        if (s.lf + 1 == n) {
            nxt.lf = nxt.push = 0;
            nxt.rt = s.rt + 1;
        } else nxt.lf = s.lf + 1;
    ...
    for (int i = 0; i < F; ++ i) {
        ...
        auto s = hash<state>{}[i];
#if UNOFFICIAL_TUTORIAL
        auto cap = min(a[s.lf] - s.flow[s.lf], b[s.rt] - s.push);
        auto now = s;
        moveto(s, now), update(now, dp[i]);
        for (int f = 1; f <= cap; ++ f) {
            now = s;
            now.flow[s.lf] += f, now.push += f;
            moveto(s, now);
            update(now, dp[i] + c[s.lf][s.rt]);
        }
#else
        for (auto f : {0, 1, 2, 3, 4}) {
            ...
            now.flow[s.lf] += f, now.push += f;
            moveto(s, now);
            update(now, (!f ? 0 : c[s.lf][s.rt]) + dp[i]);
        }
#endif
    }
    $.put(ans == inf ? -1 : ans);
