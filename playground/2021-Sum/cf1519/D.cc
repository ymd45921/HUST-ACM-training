...
signed main() {
    ...
    int n = $.nextInt();
    $.nextArray(a + 1, a + 1 + n).nextArray(b + 1, b + 1 + n);
    for (int i = 1; i <= n; ++ i)
        ab[i] = (longs)a[i] * b[i] + ab[i - 1];
    longs ans = ab[n];
    for (int l = 1; l <= n; ++ l) {
        int ll = l, rr = l;
        longs tmp = ab[ll] - ab[ll - 1];
        while (-- ll >= 1 && ++ rr <= n) {
            tmp += (longs)a[ll] * b[rr];
            tmp += (longs)a[rr] * b[ll];
            maximize(ans, tmp + ab[n] - ab[rr] + ab[ll - 1]);
        }
        if (l == n) continue;
        ll = l, rr = l + 1;
        tmp = (longs)a[ll] * b[rr] + (longs)a[rr] * b[ll];
        maximize(ans, tmp + ab[n] - ab[rr] + ab[ll - 1]);
        while (-- ll >= 1 && ++ rr <= n) {
            tmp += (longs)a[ll] * b[rr];
            tmp += (longs)a[rr] * b[ll];
