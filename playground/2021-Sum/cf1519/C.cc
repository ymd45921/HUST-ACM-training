...
signed main() {
    ...
    int T = $.nextInt(), n;
    while (T --) {
        $(n).nextArray(u + 1, u + 1 + n)
            .nextArray(s + 1, s + 1 + n);
        memset(ans, 0, sizeof(longs) * (n + 1));
        auto m = *max_element(u + 1, u + 1 + n);
        li.clear(), sum.clear();
        li.resize(m + 1), sum.resize(m + 1);
        for (int i = 1; i <= n; ++ i)
            li[u[i]].push_back(s[i]);
        for (int i = 1; i <= m; ++ i) {
            sort(li[i].begin(), li[i].end(), greater<>());
            sum[i] = li[i];
            auto siz = li[i].size();
            for (int j = 1; j < siz; ++ j)
                sum[i][j] += sum[i][j - 1];
