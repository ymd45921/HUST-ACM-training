...
longs brute(int i) {
    for (longs ii = max(a[i - 1], a[i + 1]) + 1; ii <= (int)2e9; ++ ii)
        if (gcd(a[i - 1], ii) == 1 && gcd(a[i + 1], ii) == 1) return ii;
    exit(EXIT_FAILURE);
}
...
signed main() {
    ...
    while (T --) {
        int n = $.nextInt();
        $.nextArray(a + 1, a + 1 + n);
        ans.clear(), a[0] = a[n + 1] = 1;
        for (int i = 1; i <= n; ++ i) {
            if (i > 1 && a[i] >= a[i - 1]) {
                auto g = gcd(a[i - 1], a[i + 1]);
                if (gcd(a[i], a[i - 1]) == 1 && gcd(a[i], a[i + 1]) == 1) continue;
                a[i] = brute(i);
                ans.emplace_back(i - 1, i, a[i - 1], a[i]);
...
//            }
        $.put(ans.size());
        for (auto [i, j, x, y] : ans) $.put(i, j, x, y);
    }
