 ...
 *
 * 所谓的使用优先队列进行贪心的做法
 * 只是合并并查集的外向边的时候使用启发式合并
 * 这样就可以保证合并总复杂度都是 log n 的
 * 倒也很经典，看起来是 O(n²) 实际是对数
 */
#include <bits/stdc++.h>
...
class heuristic_dsu {
    ...
    using h_func = function<bool(int, int)>;

    h_func h = [&](int u, int v) { return fa[u] <= fa[v]; };
...
public:
    ...
    explicit heuristic_dsu(int n) : fa(n, -1) {}

    heuristic_dsu(int n, h_func H) : fa(n, -1), h(std::move(H)) {}
...
signed main() {
    ...
    heuristic_dsu dsu(n + 1, [&](int u, int v) {
    ...
    vector<int> ans;
    const auto merge = [&](int aa, int bb) {
        auto fa = dsu.id(aa), fb = dsu.id(bb);
        if (dsu.connect(aa, bb)) {
            int kep = dsu.id(aa) == fa ? fa : fb;
            int dis = dsu.id(aa) == fa ? fb : fa;
            for (auto iii : g[dis]) g[kep].push_back(iii);
            g[dis].clear();
            a[kep] += a[dis] - x, a[dis] = 0;
        }
    };
    for (int i = 1; i <= n; ++ i) {
        int ii = dsu.id(i);
        while (!g[ii].empty()) {
            auto [v, id] = g[ii].back();
            int jj = dsu.id(v);
            if (ii == jj) g[ii].pop_back();
            else if (a[ii] + a[jj] < x) break;
            else {
                g[ii].pop_back();
                merge(ii, jj);
                ans.push_back(id);
