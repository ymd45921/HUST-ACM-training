 ...
 *
 * 有趣的前向星实现方法：
 * @see https://codeforces.com/blog/entry/90236 F solution
 *
 * 考虑 DFS 树上从下网上返回的过程：
 * - 因为是从下往上，所以首先会处理叶子节点和其父亲
 * - 如果可以合并，那么就直接合并好了
 * - 否则，那就是等其他部分都合并完了，它就要放在最后面
 * - 对于后考虑的叶子节点，是对于排除了前面叶子的图而言的，所以在它前面
 * 核心：排除了不够合并的叶子之后，剩下来的是少一个节点的合理的图
 */
#include <bits/stdc++.h>
...
class fws {
    ...
    struct edge {
        ...
        edge(int u, int v, T w, int x) : u(u), v(v), w(w), next(x) {}
...
signed main() {
    ...
    const function<void(int)> dfs = [&](int u) {
        vis.set(u);
        g.for_each(u, [&](int id, auto e) -> bool {
            auto [u, v, ii] = e;
            if (!vis.test(v)) {
                dfs(v);
                if (a[u] + a[v] >= x) {
